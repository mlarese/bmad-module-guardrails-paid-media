...
1. **Config.** Leggi `{project-root}/_bmad/config.toml` e `{project-root}/_bmad/config.user.toml`
   (livello radice). Se manca, dillo una volta: `gpm-setup` può installare il
...
**Se `project-profile.md` non esiste:** proponi `gpm-profile`, oppure raccogli al volo le quattro
